...
<html>
  ...
  <table width="750" border="0">
    ...
    <tr>
      <td><div align="right"><a href="proyectoAction.do?hacia=aRegNLOIni&idProy=<%=proy.getIdProy()%>">Registrar
          Nueva NLO a la Iniciativa</a></div></td>
    </tr>
    <tr>
      <td><div align="left">
          <%
      if(proy.getNLOs().size()!=0){
        request.setAttribute("nsot",proy.getNLOs());
        %>
          <display:table class="its" name="nsot" id="ns" requestURI="proyectoAction.do">
            <display:column title="Fecha de la NLO"><%=sdf.format(((NLOdeListaVO)ns).getFechaNLO())%> </display:column> <display:column title="Descripción NLO" property="str" maxWords="10">
          </display:column>
          <display:column title="Editar"> <a href="proyectoAction.do?hacia=editarNLO&idProy=<%=proy.getIdProy()%>&idNLO=<%=((NLOdeListaVO)ns).getId()%>">Editar
          NLO</a> </display:column>

          <display:column title="Eliminar"> <a href="proyectoAction.do?hacia=eliminarNLO&idNLO=<%=((NLOdeListaVO)ns).getId()%>" onClick="return confirmaEliminacion();" >Eliminar
          NLO</a> </display:column> <display:setProperty name="basic.msg.empty_list" value="No se encontraron elementos para mostrar"/>
